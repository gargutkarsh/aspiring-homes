...
function getList(){
    ...
    if (dataString) {
        ...
        url: urlbase + "president/getDHList",
        ...
        success: function (response) {
            if (response.result.length > 0) {
                ...
                for (var i = 0; i < response.result.length; i++) {
                    ...
                    table = table + "<tr><td id='cId' class='d-none'> " + response.result[i].cId + " </td><td> <img src='"+imgbase + response.result[i].profile+"' class='avatar avatar-30 mr-2' alt='pic'/> " + response.result[i].name +" </td><td> " + status + " </td><td><a href='javascript:' aid = " + response.result[i].cId + " onclick='funDHInfo(this)' ><i class='fas fa-info-circle'></i></a></td></tr>";
                }
                table = table + "</tbody>";
                $("#tblDHList").append(table);
                //bindRequestInfo();
...
function getDHInfo(){
    ...
    $.ajax({
        ...
        contentType: "application/json",
        async: true,
        url: urlbase + "dataHandler/getDHInfo/" + cId,
        data: {},
        datatype: "json",
        success: function (response) {
            if(response.found){
...
$(document).ready(function () {
    ...
    $.validator.addMethod("haveAccount", function (value) {
        ...
        var found = $.ajax({
                ...
                async: false,
                data: {},
                url: urlbase + "dataHandler/checkAccount/" + value,
                datatype: "json",
...
$("#btnCreateDataHandler").click(function (e) { 
    e.preventDefault();
    var dataString = localStorage.getItem("rup-user");
    if (dataString) {
        ...
        var data = {
         ...
         $.ajax({
            ...
            contentType: "application/json",
            async: true,
            url: urlbase + "dataHandler/insertDataHandler",
            data: JSON.stringify(data),
            datatype: "json",
            beforeSend: function () {
                $("#btnCreateDataHandler").attr("disabled",true);
                //     $("#saving").modal({
                //     show: true,
                //     backdrop: 'static',
                //     keyboard:false
            ...
            success: function (response) {
                if(response.done){
                    resetForm();
                    swal("Data Handler Created","Successfully","success");
                    getList();
                }
                //console.log(response);
            },
...
$("#btnSaveDataHandler").click(function (e) { 
    e.preventDefault();
    if($("#dataHandlerForm").valid()){
        var data = {
            cId:$("#spancId").text(),
...
$("#btnDeleteDataHandler").click(function (e) { 
    e.preventDefault();
     swal({
        title: "Are you sure?",
        ...
        if (willDelete) {
            $.ajax({
                ...
                contentType: "application/json",
                async: true,
                url: urlbase + "dataHandler/deleteDataHandler/" + $("#spancId").text(),
                data: {},
                datatype: "json",
                beforeSend: function () {
                    //     $("#saving").modal({
...
function resetForm(){
    $("#btnCreateDataHandler").attr("disabled",false);
